...
class MergeStopDialog extends React.Component {
  ...
  render() {
    ...
    const { open, intl, handleClose, sourceElement, targetElement, handleConfirm } = this.props
    ...
    const fromStopPlace = sourceElement ? `${sourceElement.name || ''} (${sourceElement.id})` : ''
    const toStopPlace = targetElement ? `${targetElement.name || ''} (${targetElement.id})` : ''
        ...
        { canMerge ? (
            <div>
              ...
              <div style={{marginTop: 10, display: 'flex', alignItems: 'center'}}>
                ...
                <span style={{fontWeight: 600, marginLeft: 5}}>{ translations.warning }</span>
              </div>
            </div>
          ) : (
