...
class DeleteQuayDialog extends React.Component {
  ...
  render() {
    const { open, intl, handleClose, deletingQuay, handleConfirm, isLoading, importedId = []} = this.props;
    ...
    return (
      ...
      >
        <div>
          ...
          <div style={{ marginLeft: 0, color: '#000', background: '#fcc8c5', padding: 10, border: '1px solid black'}}>
            {translations.info}
          </div>
          { importedId.length  ?
            <div style={{padding: 10, color: '#000', maxHeight: 200, overflow: 'auto'}}>
              <div>{translations.importedId}</div>
              <ol>{importedId.map( (entry,index) =>
                <li key={"importedId-"+index} style={{lineHeight: '1.4em'}}>{entry}</li>
              )}</ol>
            </div>
            : <p style={{fontStyle: 'italic'}}>{translations.importedId_empty}</p>
          }
          <Checkbox
            style={{marginTop: 10}}
