...
LimitationHelpers.updateCurrentWithLimitations = (current, payLoad) => {
  ...
  if (copy.quays) {
    copy.quays = copy.quays.map( quay => setLimitationForEntity(quay, limitationType, value))
  }
  return copy
}
...
LimitationHelpers.updateCurrentWithQuayLimitations = (current, payLoad) => {
  ...
  if (copy.quays && copy.quays[index]) {
    copy.quays[index] = setLimitationForEntity(copy.quays[index], limitationType, value)
    const correctLimitationValue = getLimitationForStopBasedOnQuays(copy.quays, limitationType)
...
const setLimitationForEntity = (source, limitationType, value) => {
  ...
  if (!entity.accessibilityAssessment || !entity.accessibilityAssessment.limitations) {
...
const getLimitationForStopBasedOnQuays = (quays, limitationType) => {
  ...
  if (!quays || !quays.length) return Limitations.defaultLimitations[limitationType]
  ...
  quays.forEach( q => {
    ...
    let limitation = getIn(q, ['accessibilityAssessment', 'limitations', limitationType], null)
